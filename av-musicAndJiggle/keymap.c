...
static bool key_trigger = true; //enabled by default, false to disable
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_ADJUST] = LAYOUT_planck_grid(
    KC_TRANSPARENT, KEY_TIMER, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, 
...
if (timer_elapsed32(key_timer) > 180000) { // 3 min
    key_timer = timer_read32();  // resets timer
    if (key_trigger) { // tap if enabled
      tap_code(KC_NO);
    //   tap_code(KC_MS_UP);
    //   tap_code(KC_MS_DOWN);
    //   tap_code(KC_MS_LEFT);
